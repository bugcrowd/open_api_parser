  ...
  module Specification
    class Endpoint
      ...
      def json_entry(schema, name, value)
        ...
        if properties.key?(name) && properties[name]['type'] == 'integer' && value =~ /\A[0-9]+\z/
          { name => value.to_i }
        else
          { name => value }
        end
      ...
      def parameter_schema(empty_schema, type)
        ...
        properties = type_params.reduce({}) do |schema, param|
          schema_value = param.clone.delete_if do |k, _v|
        ...
        type_schema['required'] = required_params if required_params.any?

        type_schema
      end

      def response_from_status(status)
        entry = @raw.fetch('responses', {}).detect do |k, _v|
          k.to_s == status.to_s
        end
